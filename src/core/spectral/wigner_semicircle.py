...
class WignerSemicircleDistribution:
    ...
    def __init__(self, R: float = None, *, radius: float = None) -> None:
        """
        Args:
            R      : spectral radius (positional or keyword)
            radius : alias for R (keyword-only, for API compatibility)

        One of R or radius must be provided.
        R = 2σ√N for N×N GOE matrix with entry std σ.
        """
