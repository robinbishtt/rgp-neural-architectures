"""
figures/extended_data/generate_extended_table2.py

Extended Data Table 2 — Hyperparameter Sweep Results

Comprehensive hyperparameter sweep across network widths N in [256, 1024],
depths L in [100, 500], and activation functions (tanh, swish, erf). All
configurations maintain stability markers with xi > 10 and lambda_max < 0.1.

Key findings from supplementary text:
    - Stability maintained across the entire tested parameter range
    - Optimal accuracy achieved at N=1024 with erf activation
    - Depth increase from 100 to 500 shows <= 2% accuracy degradation

Outputs: results/extended_data/extended_table2.{csv,md,json}
"""
...
def run(fast_track: bool = False) -> None:
    ...
    md = [
        "## Extended Data Table 2: Hyperparameter Sweep",
        "", "Critical init throughout. Stable = ξ>10 and λ_max<0.1.",
        "",
        "| Act | N | L | ξ | λ_max | Acc(%) | Stable |",
        "|-----|---|---|---|-------|--------|--------|",
