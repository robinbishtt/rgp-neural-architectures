"""
src/utils/fast_track_validator.py

Validates fast-track pipeline outputs to ensure sensible results
(no NaN, correct shapes, convergence detected) before marking as VERIFIED.
"""
...
class FastTrackValidator:
    """
    Validates fast-track mode outputs.
    All checks must pass before [FAST_TRACK_UNVERIFIED] is upgraded to verified.
    """
    ...
    def validate_correlation_length(
        ...
        result.add("positive_xi", (xi_values > 0).all(),
                   "Non-positive xi values detected")
