"""
tests/robustness/test_input_corruption.py

Input corruption robustness: occlusion, zeroing, and random masking.
"""
...
class TestInputCorruption:
    ...
    def test_zero_masking_finite(self, model, x):
        """Zeroing out 50% of input features must produce finite output."""
    ...
    def test_full_zero_input_finite(self, model, x):
        """All-zero input must produce finite output (bias terms active)."""
    ...
    def test_single_feature_active(self, model, x):
        """Only one active feature must produce finite output."""
    ...
    def test_random_masking_levels(self, model, x):
        """Test output finiteness across a range of masking probabilities."""
    ...
    def test_output_shape_invariant_to_corruption(self, model, x):
        """Corruption must not change output tensor shape."""
