"""tests/robustness/test_checkpoint_robustness.py"""
import pytest
import torch
import tempfile
import os
...
class TestCheckpointRobustness:
    def test_save_load_produces_identical_output(self):
        """Model saved and loaded must produce bit-exact identical predictions."""
        ...
        assert torch.allclose(out_before, out_after, atol=1e-6), \
            "Checkpoint save/load changed model outputs"

    def test_partial_state_dict_handled_gracefully(self):
        """Loading a subset of parameters should not silently corrupt the model."""
        from src.architectures.rg_net.rg_net_standard import RGNetStandard
        torch.manual_seed(7)
        model = RGNetStandard(input_dim=16, n_classes=4)
