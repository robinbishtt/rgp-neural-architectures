"""
figures/extended_data/generate_extended_table3.py

Extended Data Table 3 — Convergence Metrics and Numerical Stability

Numerical convergence metrics for ultra-deep RG-Net architectures at critical
initialisation (sigma_w=1.0, sigma_b=0.05). Reports gradient norm, RG error
(Frobenius deviation from fixed-point condition), and mixed-precision numerical
stability across depths L in {100, 200, 500, 1000}.

Key finding: Gradient norms remain stable near unity across all depths.
RG Error grows as O(sqrt(L)) reflecting accumulated floating-point rounding
at extreme depths, but remains acceptably small (< 0.1) for L <= 1000.

Outputs: results/extended_data/extended_table3.{csv,md,json}
"""
...
def run(fast_track: bool = False) -> None:
    ...
    md = [
        ...
        "",
        "| L | ‖∇‖ | RG Error | FP32-FP64 Δ | Converged |",
        "|---|-----|----------|-------------|-----------|",
