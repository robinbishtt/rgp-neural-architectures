...
Finite-size scaling data collapse quality analysis:
  a) Raw L_min vs ξ_data data for all widths (no collapse)
  b) FSS-collapsed curves with optimal ν exponent
  c) Collapse quality metric χ²_collapse vs exponent ν (minimum indicates ν*)
  d) Bootstrap distribution of ν* with 95% CI marked

Usage
-----
    python figures/extended_data/run_extended_figure9.py \
        --results results/ --output figures/out/ed_fig9.pdf
    python figures/extended_data/run_extended_figure9.py --fast-track
"""
...
def _synthetic_data(fast_track: bool = False) -> dict:
    """Generate synthetic data for Extended Data Figure 9."""
...
def _build_figure(d: dict, fast_track: bool = False) -> plt.Figure:
    """Construct the four-panel Extended Data Figure 9."""
    ...
    fig.suptitle("FSS Collapse Quality Across Widths", fontsize=9, y=1.01)
    return fig
