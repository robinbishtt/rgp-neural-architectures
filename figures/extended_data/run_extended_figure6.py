...
def _panel_perturbation_norm(ax: plt.Axes, data: Dict, fast_track: bool) -> None:
    ...
    ax.set_title("Perturbation growth" + tag, fontsize=7, pad=3)
...
def _panel_sensitivity_norm(ax: plt.Axes, data: Dict) -> None:
    ...
    ax.set_title("Cumulative Jacobian norm", fontsize=7, pad=3)
    ax.legend(frameon=False, fontsize=6)
    remove_top_right_spines(ax)
...
def _panel_xcorr(ax: plt.Axes, data: Dict) -> None:
    ...
    ax.set_title("Activation de-correlation", fontsize=7, pad=3)
    ax.legend(frameon=False, fontsize=6)
    remove_top_right_spines(ax)
...
def _panel_sv_spectrum(ax: plt.Axes, data: Dict) -> None:
    ...
    ax.set_title("Sensitivity matrix spectrum (critical)", fontsize=7, pad=3)
    ax.legend(frameon=False, fontsize=6)
    remove_top_right_spines(ax)
