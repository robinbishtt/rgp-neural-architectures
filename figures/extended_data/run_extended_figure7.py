...
Four-panel analysis of the Lyapunov exponent spectrum as a function of depth:
  a) Largest Lyapunov exponent λ_1 vs depth for critical vs off-critical init
  b) Lyapunov spectrum rank decay at L=100, 500, 1000
  c) Correlation between λ_1 and measured correlation length decay rate k_c
  d) Phase diagram: ordered vs chaotic regimes in (σ_w, σ_b) space

Usage
-----
    python figures/extended_data/run_extended_figure7.py \
        --results results/ --output figures/out/ed_fig7.pdf
    python figures/extended_data/run_extended_figure7.py --fast-track
"""
...
def _synthetic_data(fast_track: bool = False) -> dict:
    """Generate synthetic data for Extended Data Figure 7."""
...
def _build_figure(d: dict, fast_track: bool = False) -> plt.Figure:
    """Construct the four-panel Extended Data Figure 7."""
    ...
    fig.suptitle("Lyapunov Spectrum Depth Dependence", fontsize=9, y=1.01)
    return fig
