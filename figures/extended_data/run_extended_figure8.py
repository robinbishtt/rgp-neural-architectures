...
Random matrix theory validation for wide-network Jacobian spectra:
  a) Marchenko-Pastur fit for bulk eigenvalue density at L=50
  b) KS-test p-values vs network width (convergence to MP limit)
  c) Tracy-Widom edge statistics for largest singular value
  d) Number variance Σ²(L) scaling confirming spectral rigidity

Usage
-----
    python figures/extended_data/run_extended_figure8.py \
        --results results/ --output figures/out/ed_fig8.pdf
    python figures/extended_data/run_extended_figure8.py --fast-track
"""
...
def _synthetic_data(fast_track: bool = False) -> dict:
    """Generate synthetic data for Extended Data Figure 8."""
...
def _build_figure(d: dict, fast_track: bool = False) -> plt.Figure:
    """Construct the four-panel Extended Data Figure 8."""
    ...
    fig.suptitle("Random Matrix Theory Bulk Statistics", fontsize=9, y=1.01)
    return fig
