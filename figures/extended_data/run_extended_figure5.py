...
Extended Data Figure 5 — Lyapunov Spectrum Diagnostics
  ...
  c) Convergence of QR algorithm (exponent error vs iteration)
  d) Kaplan-Yorke dimension vs σ_w

Usage
-----
    python figures/extended_data/run_extended_figure5.py \
        --output figures/out/ed_fig5.pdf
"""
...
def _synthetic_ed5_data(fast_track: bool = False) -> Dict:
    ...
    return {
        ...
        "n_iter":   n_iter,
        "qr_conv":  qr_conv,
        "sw_vals":  sw_vals.tolist(),
        "ky_dims":  ky_dims.tolist(),
    }
...
def _panel_full_spectrum(ax: plt.Axes, data: Dict) -> None:
    ...
    ax.set_title("ED5a: Full Lyapunov spectrum", fontsize=7, pad=3)
...
def _panel_lyapunov_sum(ax: plt.Axes, data: Dict) -> None:
    ...
    ax.set_title("ED5b: Lyapunov sum vs depth", fontsize=7, pad=3)
    ax.legend(frameon=False, fontsize=6)
    remove_top_right_spines(ax)
...
def _panel_qr_convergence(ax: plt.Axes, data: Dict) -> None:
    ...
    ax.set_title("ED5c: QR algorithm convergence", fontsize=7, pad=3)
    ax.legend(frameon=False, fontsize=6)
    remove_top_right_spines(ax)
...
def _panel_kaplan_yorke(ax: plt.Axes, data: Dict) -> None:
    ...
    ax.set_title("ED5d: $D_{KY}$ vs $\\sigma_w$", fontsize=7, pad=3)
    ax.legend(frameon=False, fontsize=6)
    remove_top_right_spines(ax)
