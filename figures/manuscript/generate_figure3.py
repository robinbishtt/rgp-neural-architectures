...
def _panel_xi_decay(ax: plt.Axes, data: Dict, fast_track: bool) -> None:
    ...
    ax.set_title(r"Correlation-length decay" + tag, fontsize=7, pad=3)
...
def _panel_ks_pvalues(ax: plt.Axes, data: Dict, fast_track: bool) -> None:
    ...
    ax.set_ylim(0, 1.05)
    tag = "" if fast_track else ""
    ax.set_title("MP fit quality (KS test)" + tag, fontsize=7, pad=3)
...
def _panel_chi1_phase(ax: plt.Axes, data: Dict) -> None:
    ...
    ax.set_ylim(0, 1.15)
    ax.set_title(r"$\chi_1$ phase diagram", fontsize=7, pad=3)
