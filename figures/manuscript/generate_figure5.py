...
def _synthetic_h3_data(fast_track: bool = False) -> Dict:
    ...
    return {
        :      _MODELS,
        :    {k: v.tolist() for k, v in iid_runs.items()},
        :   {k: v.tolist() for k, v in hier_runs.items()},
        :  ood_curves,
        :  shifts.tolist(),
        :   pvals_iid,
        :  pvals_hier,
...
def _panel_accuracy_bars(ax: plt.Axes, data: Dict, fast_track: bool) -> None:
    ...
    ax.set_title("RG-Net vs baselines" + tag, fontsize=7, pad=3)
...
def _panel_ood_generalisation(ax: plt.Axes, data: Dict, fast_track: bool) -> None:
    ...
    tag = "" if fast_track else ""
    ax.set_title("OOD generalisation" + tag, fontsize=7, pad=3)
...
def _panel_significance_heatmap(ax: plt.Axes, data: Dict) -> None:
    ...
    pmat = np.vstack([pvals_iid, pvals_hier])   
    ...
    ax.set_title("Wilcoxon significance\n(RG-Net > baseline)", fontsize=7, pad=3)
